...
class Game():
    ...
    def __init__(self, m, n, mines, res=750):

        # initialise game
        os.environ['SDL_VIDEO_CENTERED'] = '1'
        pygame.init()
        self.window = pygame.display.set_mode((res, res))
        ...
        self.ref_grid = self.create_reference()
        ...
        # load sprites
        self.cell_size = min([int(res / n), int(res / m)])
        self.sprite1 = pygame.transform.scale(pygame.image.load("./Sprites/one.png"), (self.cell_size, self.cell_size))
        self.sprite2 = pygame.transform.scale(pygame.image.load("./Sprites/two.png"), (self.cell_size, self.cell_size))
        self.sprite3 = pygame.transform.scale(pygame.image.load("./Sprites/three.png"), (self.cell_size, self.cell_size))
        self.sprite4 = pygame.transform.scale(pygame.image.load("./Sprites/four.png"), (self.cell_size, self.cell_size))
        self.sprite5 = pygame.transform.scale(pygame.image.load("./Sprites/five.png"), (self.cell_size, self.cell_size))
        self.sprite6 = pygame.transform.scale(pygame.image.load("./Sprites/six.png"), (self.cell_size, self.cell_size))
        self.sprite7 = pygame.transform.scale(pygame.image.load("./Sprites/seven.png"), (self.cell_size, self.cell_size))
        self.sprite8 = pygame.transform.scale(pygame.image.load("./Sprites/eight.png"), (self.cell_size, self.cell_size))
        self.spritef = pygame.transform.scale(pygame.image.load("./Sprites/flag.png"), (self.cell_size, self.cell_size))
        self.spriterevealed = pygame.transform.scale(pygame.image.load("./Sprites/revealed.png"), (self.cell_size, self.cell_size))
        self.spriteunrevealed = pygame.transform.scale(pygame.image.load("./Sprites/unrevealed.png"), (self.cell_size, self.cell_size))

    def count_mines(self, i, j, grid):
    ...
    def create_reference(self):
        ...
        arr = np.array([-1] * self.mines + [0] * (self.m * self.n - self.mines))
        # randomise locations
        np.random.shuffle(arr)
        # reshape into m x n grid
        ref_grid = np.reshape(arr, (self.m, self.n))
        ...
        for i in range(0, self.m):
            for j in range(0, self.n):
                ...
                if ref_grid[i, j] != -1:
                    ...
                    # and col
                    ref_grid[i, j] = self.count_mines(i + 1, j + 1, ref_copy)
        return ref_grid
    ...
    def zero_method(self, x, y, ref_grid, player_grid):
    ...
    def update(self):
        ...
        if self.type == "f":
            ...
            # otherwise: add flag (only if an unrevealed square)
            elif entry == "-":
        ...
        # check normal move: explode mine or record info on player grid
            ...
            # if not mine: display value
                ...
                if self.ref_grid[i, j] == 0:
                    self.ref_grid, self.player_grid = self.zero_method(i, j, self.ref_grid, self.player_grid)
                # if not, call on any neighbouring zeros
                else:
                    nbs = self.neighbours(i, j)
                    for pos in nbs:
                        if self.ref_grid[pos] == 0:
                            self.ref_grid, self.player_grid = self.zero_method(pos[0], pos[1], self.ref_grid, self.player_grid)
